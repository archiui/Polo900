    ...
    jQuery(function() {
        ...
        archiuitree = jQuery('#archiuitree').jstree({
            ...
            if (typeof jQuery('#archiuitree #' + node.node.id + ' .controls .fa.fa-pencil').data('events') === 'undefined' || typeof jQuery('#archiuitree #' + node.node.id + ' .controls .fa.fa-pencil').data('events').click === 'undefined') {
                ...
                jQuery('#archiuitree #' + node.node.id + ' .controls .fa-sort-amount-desc').first().on('click', selectAllNode);
            }
            ...
            var children = archiuitree.jstree().get_children_dom(selected.node);
            if (jQuery('#archiuitree #' + selected.node.id + ' .controls .fa-sort-amount-desc').first().hasClass('attivo')) {
                jQuery('#archiuitree #' + selected.node.id + ' .controls .fa-sort-amount-desc').first().removeClass('attivo');
                // Deseleziona tutti i figli
                var i = 0;
                while (i < children.length) {
                    var elem = children.get(i++);
                    if (!jQuery(elem).hasClass('jstree-leaf')) {
                        jQuery(elem).find('.controls .fa-sort-amount-desc').removeClass('attivo');
                    }
                    children = jQuery.merge(children, archiuitree.jstree().get_children_dom(elem));
                }
            }
            ...
            var i = 0;
            var children = archiuitree.jstree().get_children_dom(node.node);
            while (i < children.length) {
                var elem = children.get(i++);
                archiuitree.jstree().select_node(elem);
                if (!jQuery(elem).hasClass('jstree-leaf')) {
                    jQuery(elem).find('.controls .fa-sort-amount-desc').addClass('attivo');
                    var jstreeNode = archiuitree.jstree().get_node(node);
                }
                children = jQuery.merge(children, archiuitree.jstree().get_children_dom(elem));
            }
        });
        ...
        jQuery('#archiuitree').on('click', '.controls .fa-sort-amount-desc', selectAllNode);
        ...
        function selectNode(e) {
         ...
         function selectAllNode(e) {
            var node = jQuery(this).parents('.jstree-node').first();
            // var jstreeNode = archiuitree.jstree().get_node(node);
            if (!jQuery(this).parents('.treenode-cont').hasClass('jstree-clicked')) {
                return;
            }
            var children = archiuitree.jstree().get_children_dom(node);
            if (jQuery(this).hasClass('attivo')) {
                jQuery(this).removeClass('attivo');
                // jstreeNode.state.selectAll = false;
                // Deseleziona tutti i figli
                var i = 0;
                while (i < children.length) {
                    var elem = children.get(i++);
        ...
        function manageAction(node)    {
            if (node.selected.length >= 1)  {
                ...
                jQuery('#btnPubblica, #btnNascondi').removeClass('disable');
            } else {
                jQuery('#btnElimina, #btnTaglia').addClass('disable');
                jQuery('#btnPubblica, #btnNascondi').addClass('disable');
            }
