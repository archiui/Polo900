...
class StrumentiController extends ActionController {
    ...
    public function AllNode($pa_values = NULL, $pa_options = NULL) {
        ...
        if ($user->canDoAction('is_administrator')) {
            $query = "
              SELECT t.object_id as id, t.access as access, t.parent_id,t.type_id as type, l.name as text, (SELECT COUNT(*) FROM ca_objects p WHERE t.object_id = p.parent_id) hasChildren
        ...
        while ($qr_result->nextRow()) {
            ...
            if ($access) {
                $nodo->text .= "<span class='occhietto fa fa-eye'></span>";
            }
            $return[] = $nodo;
            $i++;
        }
    ...
    public function Rifinisci($pa_values = NULL, $pa_options = NULL) {
     ...
     public function Pubblica($pa_values = NULL, $pa_options = NULL) {
        $ho_dei_padri = false;
        if (!isset($_POST) || !isset($_POST['ids']) || !isset($_POST['access']))  {
            return "Errore";
        } else {
            $parent_id = array();
            foreach ($_POST['ids'] as $object_id) {
                $this->transiction['UPDATE'][ $object_id['id'] ]['intr'] = array( 'access' => (int) $_POST['access'] );
                if ($object_id['parent'] == 'true') {
        ...
        $count = 1;
        while ($count > 0 && $ho_dei_padri) {
            $qr_result = $this->o_db->query("SELECT object_id FROM ca_objects WHERE deleted = 0 AND parent_id IN ({$parent_id})");
            if ($qr_result && ($count = $qr_result->numRows()) > 0 ) {
                $allObj =  $qr_result->getAllFieldValues('object_id');
                foreach ($allObj as $object_id) {
                    $this->transiction['UPDATE'][ $object_id ]['intr'] = array( 'access' => (int) $_POST['access'] );
                }
                $this->o_db->query("UPDATE ca_objects SET access = 1 WHERE object_id IN ({$allObj})");
                $parent_id = implode(',',$allObj);
            } else {
                $ho_dei_padri = false;
            }
        }
        $this->saveTrans();
        if ($_POST['access'] == '1') {
            echo "Gli elementi selezionati sono visibili online, salva per confermare";
        } else {
            echo "Gli elementi selezionati non sono visibili online, salva per confermare";
        }
    }
    ...
    public function Save($pa_values = NULL, $pa_options = NULL) {
        if (isset($_GET['background']) && $_GET['background'] == 'true') {
            $command = "php " . $this->plugin_path . "tools/tasksave.php > /dev/null 2>/dev/null &";
            exec($command);
            echo "Le modifice verranno salvate in background, ora puoi continuare a navigare";
        } else {
            $transazione = new Transaction($this->o_db, MYSQLI_TRANS_START_WITH_CONSISTENT_SNAPSHOT);
            $this->runTransaction($transazione, false, true, true);
    ...
    private function rifinisciRicorsiva($id, $accumulatore, $mapRank = array()) {
        ...
        foreach ($children as $child) {
            $new_accumulatore = $this->rifinisciRicorsiva($child, $temp_accumulatore, $mapRank);
            $accumulatore = $this->accumulatoreMerge($accumulatore, $new_accumulatore);
        }
     ...
     private function accumulatoreMerge($accumulatore, $new_accumulatore) {
        $tmp_accumulatore = array();
        $tmp_accumulatore['inizio'] =  ((int)$accumulatore['inizio'] < (int)$new_accumulatore['inizio']) ? $accumulatore['inizio'] :  $new_accumulatore['inizio'];
        $tmp_accumulatore['fine'] =  ((int)$accumulatore['fine'] > (int)$new_accumulatore['fine']) ? $accumulatore['fine'] :  $new_accumulatore['fine'];

        foreach ($accumulatore['tipologia'] as $tipo => $count) {
            if (isset($tmp_accumulatore['tipologia'][$tipo])) {
                $tmp_accumulatore['tipologia'][$tipo]['count'] += $count['count'];
            } else {
                $tmp_accumulatore['tipologia'][$tipo] = $count;
            }
        }

        foreach ($new_accumulatore['tipologia'] as $tipo => $count) {
            if (isset($tmp_accumulatore['tipologia'][$tipo])) {
                $tmp_accumulatore['tipologia'][$tipo]['count'] += $count['count'];
            } else {
                $tmp_accumulatore['tipologia'][$tipo] = $count;
